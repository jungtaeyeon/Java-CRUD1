...
public class CustomerManager extends JFrame implements ActionListener {
	...
	public boolean customerInsert(CustomerVO cVO) {
		...
		jt_c_birthday.setText("");
		dtm.addRow(input);
		cVO.setC_id(Integer.parseInt(input[0]));   // 여기 C_id는 int인데 input[0]이 String타입이라 Integer.parseInt() 를 사용하여 바꿔줌
...
//		input[0] = se_c_id.getText();
		input[0] = Integer.toString(cVO.getC_id()); // TextFiled에 입력한 숫자가 아닌 VO에서 받아온 값으로 처리하는걸로 바꿈
...
//		cusomerDialog = new CustomerDialog();
		cVO = new CustomerVO();
		...
		if (obj == jbtnAllSelect) { // 전체조회
			...
			if (jt_customer.getSelectedRow() == -1) { // -1일때(아무것도 선택되지 않았을때
				...
				int selectRowNum = (int)dtm.getValueAt(jt_customer.getSelectedRow(),0);  // getValueAt가 Object 타입이라 int타입으로 바꿔줌
				cVO.setC_id(selectRowNum);
				customerDelete(cVO);
				dtm.removeRow(jt_customer.getSelectedRow()); // 선택된 행값을 가져와서 삭제메소드실행
			}
			
		} else if (obj == jbtnSelect) { // 한 건 조회
			cVO.setC_id(Integer.parseInt(se_c_id.getText()));
			...
			if (jt_customer.getSelectedRow() == -1) { // -1일때(아무것도 선택되지 않았을때
				...
				customerDialog.initDisplay(jt_customer.getSelectedRow());
			}
		}
	}
}